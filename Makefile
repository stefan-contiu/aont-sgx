...
is_sgx         :=$(SGX:0=)
SrcDir         := src
SGX_COMMONDIR  := $(realpath ../sgx/sgx_common)
...
OSSLDir         := $(realpath ../sgx/intel-sgx-ssl)
...
Enclave_IncDirs := $(SGX_SDK)/include/stlport $(SrcDir) $(SGX_SDK)/include \
                   ...
                   $(SGXOSSLIDir)
Encl_Include    := $(addprefix -I, $(Enclave_IncDirs))
...
$(ObjDir)/$(EnclaveName)_t.o : $(SGXOpenSSLLib)
else
#IBBEC          := $(IBBEC:%.c=%_u.o)
#IBBECXX        := $(IBBECXX:%.cpp=%_u.o) admin_api_u.o tests_u.o
#IBBEObjs       := $(IBBEC) $(IBBECXX)
...
NatvIncludeDirs += $(SrcDir) $(SGX_COMMONDIR)
...
EnclGenericComp := $(filter-out $(ObjDir)/$(EnclaveName)_t.o $(EnclCommonObjs), $(EnclAllObjs))
